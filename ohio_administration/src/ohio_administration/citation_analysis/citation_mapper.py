#!/usr/bin/env python3
"""
Citation graph builder for Ohio Administrative Code corpus
Extracts explicit rule references and builds dependency mappings
"""
...
class CitationMapper:
    def __init__(self, input_file: str, output_dir: str = "citation_analysis"):
        ...
        # Reference extraction patterns for Ohio Administrative Code
        self.reference_patterns = [
            r'(?:O\.A\.C\.|Ohio\s+Adm(?:\.|ministrative)\s*Code)\s+([\d:\-]+)',  # O.A.C. 3701-17-01
            r'[Rr]ule\s+([\d\-:]+)',  # rule 3701-17-01
            r'([\d]{3,4}\-[\d]{1,2}\-[\d]{1,2})',  # Direct format: 3701-17-01
            r'[Ss]ection\s+(\d+\.\d+)(?:\s+of\s+the\s+Revised\s+Code)?',  # Cross-corpus: section 119.01
            r'(?<![.\d\-])(\d{3,4}\.\d+)(?![.\d\-])',  # Revised Code references: 5907.01
        ]
    ...
    def extract_section_number(header: str) -> Optional[str]:
        """Extract rule number from header (Ohio Admin Code format)"""
        # Try "Rule 3701-17-01" format first
        match = re.search(r'Rule\s+([\d\-]+)', header, re.IGNORECASE)
        if match:
            return match.group(1)
        # Fallback to direct format
        match = re.search(r'([\d]{3,4}\-[\d]{1,2}\-[\d]{1,2})', header)
    ...
    def extract_cross_references(self, text: str) -> Set[str]:
        ...
        # Clean and validate references (both Admin Code and Revised Code formats)
        valid_refs = set()
        for ref in references:
            ref = ref.strip()
            # Admin Code format: 3701-17-01
            if re.match(r'^\d{3,4}-\d{1,2}-\d{1,2}$', ref):
                valid_refs.add(ref)
            # Revised Code format (cross-corpus): 5907.01
            elif re.match(r'^\d{3,4}\.\d+$', ref):
                valid_refs.add(ref)
    ...
    def _print_analysis_summary(self, analysis: CitationAnalysis):
        ...
        print(f"\n{'=' * 60}")
        print(f"OHIO ADMINISTRATIVE CODE CITATION ANALYSIS")
        print(f"{'=' * 60}")
...
if __name__ == "__main__":
    # Ohio Administrative Code paths
    ...
    print(f"Input file: {CORPUS_FILE}")
    print(f"Output dir: {CITATION_ANALYSIS_DIR}")
    ...
    print(f"\nNext Steps:")
    print(f"1. Citation map saved to: {mapper.citation_map_file}")
    print(f"2. Complex chains saved to: {mapper.complex_chains_file}")
    print(f"3. Ready to run LMDB build with these citation files")